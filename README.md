...
네이버 지도 위에 **현위치 30/50/100m 이내 신호등**의 녹색/적색 상태와 잔여 시간을 보여주는 도보용 PWA.
...
- 지도는 **네이버 지도 JS API v3**. 클라이언트 ID는 `/api/config` 로 서버에서 내려준다(미설정 시 지도 대신 오류).
- 반경 **30 / 50 / 100m** 버튼으로 선택. 그 안의 교차로만 신호를 조회.
- 마커: 녹색 = 보행 녹색 + 잔여초, 적색 = 적색. 하단 시트에 가까운 순 목록.
...
| 경로 | 역할 |
|------|------|
| `index.html` / `style.css` / `app.js` | 네이버 지도 + 반경 선택 + 하단 목록 + 오류 배너 |
...
1. **NAVER Cloud Platform**([console.ncloud.com](https://console.ncloud.com)) 가입, 결제수단 등록.
2. Console → **Services → Application** → **Application 등록**.
3. **Maps** 카테고리에서 **Web Dynamic Map** 선택(+ 필요 시 Geocoding 등).
4. **Web 서비스 URL** 등록: `http://localhost:8000`, `https://<app>.vercel.app`.
5. 발급된 **Client ID** 를 환경변수 `NAVER_MAP_CLIENT_ID` 로.
6. 참고: 콘솔 개편으로 스크립트 파라미터가 `ncpClientId` → **`ncpKeyId`** 로 바뀜(이 앱은 `ncpKeyId`).
...
|-----|------|
| `/` | 지도 웹앱 |
| `/api/health` | `{"ok":true,"map_key":true,"signal_key":true,"naver_map_key":true}` |
...
cp .env.example .env      # .env 에 SEOUL_API_KEY, NAVER_MAP_CLIENT_ID 입력
